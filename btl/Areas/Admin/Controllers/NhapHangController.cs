...
namespace BaiTapLon.Areas.Admin.Controllers
{
    public class NhapHangController : BaseController
    {
        ...
        {
            var model = new NhapHangDraw().nhapHangView(searchString, page, pagesize);
            return View(model);
        }
        public ActionResult Duyet(string searchString, int page = 1, int pagesize = 5)
        {
            var model = new NhapHangDraw().DuyetView(searchString, page, pagesize);
            return View(model);
        }

        public ActionResult NhapKho(string searchString, int page = 1, int pagesize = 5)
        {
            var model = new NhapHangDraw().nhapKhoView(searchString, page, pagesize);
            return View(model);
        }
        public ActionResult HoanThanh(string searchString, int page = 1, int pagesize = 5)
        {
            var model = new NhapHangDraw().nhapHoanThanhView(searchString, page, pagesize);
            return View(model);
        }
        [HttpGet]
