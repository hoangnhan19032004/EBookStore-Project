...
namespace BaiTapLon.Areas.Admin.Controllers
{
    public class SanPhamController : BaseController
    {
        ...
        {
            var model = new SanphamDraw().listTheloai(searchString, page, pagesize);
            return View(model);
        }
        public ActionResult LocSach(string searchString, int page = 1, int pagesize = 5)
        {
            var model = new SanphamDraw().sachcu(searchString, page, pagesize);
            return View(model);
        }
        public ActionResult LocSachHetHang(string searchString, int page = 1, int pagesize = 5)
        {
            var model = new SanphamDraw().hetHang(searchString, page, pagesize);
            return View(model);
        }
        public ActionResult TaoDon(string searchString, int page = 1, int pagesize = 5)
        {
            var model = new SanphamDraw().listProduct(searchString, page, pagesize);
            return View(model);
        }
        public ActionResult KhoHang(string searchString, int page = 1, int pagesize = 5) {
            var model = new SanphamDraw().listKhoHang(searchString, page, pagesize);
            return View(model);
        ...
        public ActionResult ExportExelTonKho(string fromDate, string toDate)
        {
            ...
            var wb = new XLWorkbook(@"D:/PROJECT_CANHAN/websitebansach (2)/websitebansach (2)/websitebansach/websitebansach/btl/Resource/Template/Tempalet_Kho_Hang.xlsx");
